...
The integration tests need a nats-server binary (2.12+ for counter support).
...
# Counter streams (ADR-49) require nats-server 2.12+.
_VERSION = _server_version()
_COUNTERS_SUPPORTED = _VERSION is not None and _VERSION >= (2, 12, 0)
...
async def jetstream(tmp_path: Path) -> AsyncIterator[JetStream]:
    ...
    if server_bin is None or not _COUNTERS_SUPPORTED:
        pytest.skip("nats-server 2.12+ is required for counter integration tests")
